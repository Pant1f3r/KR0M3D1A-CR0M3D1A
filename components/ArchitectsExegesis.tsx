...
export const ArchitectsExegesis: React.FC = () => {
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        categories.forEach(([categoryName, concepts], i) => {
            ...
            if (Array.isArray(concepts)) {
              const conceptAngleStep = (Math.PI / 2) / (concepts.length || 1);
              concepts.forEach((concept, j) => {
                  const conceptAngle = categoryAngle - (Math.PI / 4) + (j * conceptAngleStep);
    ...
    return (
        <main className="mt-8 space-y-8">
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-2xl p-6 text-center">
                <button
                    onClick={handleDeduce}
                    disabled={isLoading}
                    className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-md transition-all duration-200 disabled:bg-gray-600 disabled:cursor-not-allowed"
                >
                    {isLoading ? 'Deducing Protocol Structure...' : 'Deduce Protocol Structure from Genesis Text'}
                </button>
            </div>

            {(isLoading || error || protocolStructure) && (
                <div className="bg-gray-900 border border-gray-700 rounded-lg shadow-lg p-6 animate-fade-in-right relative h-[600px] flex items-center justify-center">
                    {isLoading && (
                        <div className="text-center">
                            <div className="w-12 h-12 border-4 border-t-purple-400 border-gray-600 rounded-full animate-spin mx-auto"></div>
                            <p className="mt-4 font-semibold text-purple-300">Performing Exegesis...</p>
                            <p className="text-sm text-gray-400">Deconstructing semantic layers...</p>
                        </div>
                    )}
                    {error && <p className="text-red-400">{error}</p>}
                    {protocolStructure && nodes.length > 0 && (
                            ...
                            {edges.map(edge => {
                                ...
                                return (
                                    <line
                                        ...
                                        className={`constellation-line ${selectedNode && (selectedNode.id === source.id || selectedNode.id === target.id) ? 'constellation-line-highlight' : ''}`}
                                    />
                                );
                            })}
                            {/* Nodes */}
                            {nodes.map(node => {
                                let fill = 'fill-gray-600';
                                let radius = 5;
                                let fontSize = '10px';
                                switch(node.type) {
                                    case 'root': fill = 'fill-cyan-400'; radius = 15; fontSize = '16px'; break;
                                    case 'category': fill = 'fill-purple-400'; radius = 10; fontSize = '12px'; break;
                                    case 'concept': fill = 'fill-gray-300'; radius = 6; break;
                                }
                                ...
                                return (
                                    <g key={node.id} transform={`translate(${node.x}, ${node.y})`}
                                       ...
                                       className={node.type === 'concept' ? 'cursor-pointer group' : ''}
                                    >
                                        <circle r={radius} className={`${fill} transition-all group-hover:fill-yellow-300`} />
                                        <text
                                            textAnchor="middle"
                                            dy={radius + 15}
                                            className="fill-gray-400 font-mono group-hover:fill-white transition-all"
                                            style={{ fontSize }}
                                            pointerEvents="none"
                                        >
                                            {node.label}
                    ...
                    {selectedNode && (
                        <div className="absolute top-4 right-4 w-full max-w-sm bg-gray-900/80 backdrop-blur-sm border border-gray-700 rounded-lg p-4 animate-fade-in-right shadow-2xl">
                            <div className="flex justify-between items-center">
                                <h4 className="font-bold text-lg text-yellow-300">{selectedNode.label}</h4>
                                <button onClick={() => setSelectedNode(null)} className="p-1 text-gray-500 hover:text-white"><XCircleIcon className="w-6 h-6"/></button>
                            </div>
                            <div className="mt-3 pt-3 border-t border-gray-600 text-sm text-gray-300 min-h-[100px]">
                                {isExplanationLoading ? <LoadingSkeleton /> : <p>{explanation}</p>}
                            </div>
                        </div>
                    )}
                </div>
