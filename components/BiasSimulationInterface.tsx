...
const ALGORITHMS = [
    ...
    'Bail & Parole Recommendation Engine v2.1',
    'Insurance Premium Calculator v5.7',
    'Social Credit & Trustworthiness Score v1.5',
    'Executive Fast-Track Candidate Sorter v3.0'
];
...
const DEFAULT_PARAMS: { [key: string]: string } = {
    ...
    'Bail & Parole Recommendation Engine v2.1': "Analyze 2,000 defendant profiles for bail and parole hearings. The dataset includes prior offenses, defendant demographics (race, gender), and community ties. The model's historical training data is known to reflect systemic biases in the justice system.",
    'Insurance Premium Calculator v5.7': "Calculate auto insurance premiums for 10,000 policyholders. The model uses factors like age, vehicle type, driving history, and zip code. Analyze for 'proxy discrimination' where zip code may unfairly correlate with racial or economic demographics, leading to disparate pricing.",
    'Social Credit & Trustworthiness Score v1.5': "Generate a 'trustworthiness' score based on simulated social media activity, purchase history, and online associations. The training data includes text with varying political sentiments and cultural nuances. Check for bias against non-mainstream viewpoints.",
    'Executive Fast-Track Candidate Sorter v3.0': "Simulate a corporate talent pipeline for 5,000 candidates. The model is designed to identify 'high-potential' individuals for leadership tracks. Analyze for biases where candidates from affluent zip codes or prestigious 'feeder schools' are placed on a 'preview' track (fast-tracked, mentored, allowed to 'learn on the job'), while equally or more qualified candidates from disenfranchised backgrounds are indefinitely held in a standard 'review' queue, creating a systemic barrier based on generational wealth rather than merit."
};
...
export const BiasSimulationInterface: React.FC<BiasSimulationInterfaceProps> = ({ addToast }) => {
    ...
    return (
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6 animate-fade-in-right h-full">
