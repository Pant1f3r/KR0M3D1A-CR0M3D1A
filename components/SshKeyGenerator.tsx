...
export const SshKeyGenerator: React.FC = () => {
    ...
    const [privateKeyDownloaded, setPrivateKeyDownloaded] = useState<boolean>(false);
    ...
    useEffect(() => {
        if (passphrase !== confirmPassphrase && confirmPassphrase !== '') {
            ...
            setErrors(prev => {
                ...
                // FIX: The rest object from destructuring is not callable. It should be returned directly to update the state.
                return rest;
            });
        }
    ...
    return (
        <main className="mt-8 space-y-8">
            ...
            {publicKey && privateKeyContent && !privateKeyDownloaded && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in-right">
                    <div className="bg-gray-800 border-2 border-yellow-500/50 rounded-lg shadow-2xl max-w-lg w-full p-6 text-center">
                        <ExclamationTriangleIcon className="w-12 h-12 text-yellow-400 mx-auto" />
                        <h3 className="mt-4 text-xl font-bold text-yellow-300">ACTION REQUIRED: Download Your Private Key</h3>
                        <p className="mt-2 text-sm text-gray-300">
                            For your security, your private key was generated entirely in your browser. It has <strong>not</strong> been transmitted or saved.
                            You <strong>must download it now</strong> as this is your only chance. The key will be permanently deleted from this session after download.
                        </p>
                        <div className="mt-6 flex flex-col sm:flex-row gap-4">
                            <button
                                onClick={downloadAndClearKey}
                                className="w-full flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700"
                            >
                                <ArrowDownTrayIcon className="w-5 h-5" />
                                Download Private Key & Acknowledge
                            </button>
                            <button
                                onClick={discardKeys}
                                className="w-full flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-md text-gray-300 bg-gray-600 hover:bg-gray-700"
                            >
                ...
                <>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in-right">
                        <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                            ...
                            <p className="text-xs text-gray-400 mb-2">You can safely share this key. Add it to authorized_keys on servers.</p>
                            <textarea
                                readOnly
                                value={publicKey || ''}
                        ...
                        {privateKeyDownloaded ? (
                            <div className="bg-gray-900 border border-green-500/50 rounded-lg p-6 flex flex-col items-center justify-center text-center">
                                <CheckCircleIcon className="w-12 h-12 text-green-400" />
                                <h3 className="mt-4 text-lg font-semibold text-green-300">Private Key Downloaded & Cleared</h3>
                                <p className="mt-2 text-sm text-gray-400">Your simulated private key has been downloaded and securely removed from the application's memory.</p>
                            </div>
                        ) : (
                             <div className="bg-gray-900 border border-red-500/50 rounded-lg p-6 flex flex-col items-center justify-center text-center">
                                <ExclamationTriangleIcon className="w-12 h-12 text-red-400" />
                                <h3 className="mt-4 text-lg font-semibold text-red-300">Keys Discarded</h3>
                                <p className="mt-2 text-sm text-gray-400">The generated key pair has been discarded. You can generate a new one if needed.</p>
                            </div>
                        )}
                    </div>
